...
from odoo.addons.ms_google_calendar.utils.google_event import GoogleEvent
from odoo.addons.ms_google_calendar.utils.google_calendar import GoogleCalendarService
...
class GoogleSync(models.AbstractModel):
    ...
    @api.model
    def _get_sync_partner(self, emails): #this function creates res.partner record
        ...
        # if remaining:
        #     partners += self.env['mail.thread']._mail_find_partner_from_emails(remaining, records=self, force_create=True)   #this line creates res.partner record
